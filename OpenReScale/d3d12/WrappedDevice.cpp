...
bool RefCountD3D12Object::HandleWrap(const char* ifaceName, REFIID riid, void** ppvObject)
{
	...
	{
		Logger::LogInfo() << "RefCountDXGIObject.HandleWrap Querying Unknown_uuid, returning false" << std::endl;
	}
	else if (riid == ID3D12DeviceInternal2_uuid)
	{
		Logger::LogInfo() << "RefCountDXGIObject.HandleWrap Querying ID3D12DeviceInternal2_uuid, returning false" << std::endl;
	}
	else
	{
		std::string str(ifaceName);
		Logger::LogInfo() << "RefCountDXGIObject.HandleWrap Querying " + str + "  for unrecognized GUID: " << std::endl;
	}
...
{
	Logger::LogInfo() << "RefCountD3D12Object.WrapQueryInterface" << std::endl;
...
{
	Logger::LogInfo() << "D3D12Device.QueryInterface:" << std::endl;
	...
	if (riid == __uuidof(ID3D12Device))
	{
		Logger::LogInfo() << "D3D12Device.QueryInterface: Looking for ID3D12Device, returning this" << std::endl;
		AddRef();
		*ppvObject = this;
		return S_OK;
	}
	else if (riid == __uuidof(ID3D12Device1))
	{
		...
		{
			Logger::LogInfo() << "D3D12Device.QueryInterface: m_device1 is not available, returning E_NOINTERFACE" << std::endl;
			return E_NOINTERFACE;
		}

		Logger::LogInfo() << "D3D12Device.QueryInterface: Looking for ID3D12Device1, returning this" << std::endl;
		AddRef();
		*ppvObject = this;
		return S_OK;
	}
	else if (riid == __uuidof(ID3D12Device2))
	{
		...
		{
			Logger::LogInfo() << "D3D12Device.QueryInterface: m_device2 is not available, returning E_NOINTERFACE" << std::endl;
			return E_NOINTERFACE;
		}

		Logger::LogInfo() << "D3D12Device.QueryInterface: Looking for ID3D12Device2, returning this" << std::endl;
		AddRef();
		*ppvObject = this;
		return S_OK;
	}
	else if (riid == __uuidof(ID3D12Device3))
	{
		if (m_device == nullptr)
		{
			Logger::LogInfo() << "D3D12Device.QueryInterface: m_device3 is not available, returning E_NOINTERFACE" << std::endl;
			return E_NOINTERFACE;
		}

		Logger::LogInfo() << "D3D12Device.QueryInterface: Looking for ID3D12Device3, returning this" << std::endl;
		AddRef();
		*ppvObject = this;
		return S_OK;
	}
	else if (riid == __uuidof(ID3D12Device4))
	{
		...
		{
			Logger::LogInfo() << "D3D12Device.QueryInterface: m_device4 is not available, returning E_NOINTERFACE" << std::endl;
			return E_NOINTERFACE;
		}

		Logger::LogInfo() << "D3D12Device.QueryInterface: Looking for ID3D12Device4, returning this" << std::endl;
		AddRef();
		*ppvObject = this;
		return S_OK;
	}
	else if (riid == __uuidof(ID3D12Device5))
	{
		...
		{
			Logger::LogInfo() << "D3D12Device.QueryInterface: m_device5 is not available, returning E_NOINTERFACE" << std::endl;
			return E_NOINTERFACE;
		}

		Logger::LogInfo() << "D3D12Device.QueryInterface: Looking for ID3D12Device5, returning this" << std::endl;
		AddRef();
		*ppvObject = this;
		return S_OK;
	}
	else if (riid == __uuidof(ID3D12Device6))
	{
		...
		{
			Logger::LogInfo() << "D3D12Device.QueryInterface: m_device6 is not available, returning E_NOINTERFACE" << std::endl;
			return E_NOINTERFACE;
		}

		Logger::LogInfo() << "D3D12Device.QueryInterface: Looking for ID3D12Device6, returning this" << std::endl;
		AddRef();
		*ppvObject = this;
		return S_OK;
	}
	else if (riid == __uuidof(ID3D12Device7))
	{
		...
		{
			Logger::LogInfo() << "D3D12Device.QueryInterface: m_device7 is not available, returning E_NOINTERFACE" << std::endl;
			return E_NOINTERFACE;
		}

		Logger::LogInfo() << "D3D12Device.QueryInterface: Looking for ID3D12Device7, returning this" << std::endl;
		AddRef();
		*ppvObject = this;
		return S_OK;
	}
	else if (riid == __uuidof(ID3D12Device8))
	{
		...
		{
			Logger::LogInfo() << "D3D12Device.QueryInterface: m_device8 is not available, returning E_NOINTERFACE" << std::endl;
			return E_NOINTERFACE;
		}

		Logger::LogInfo() << "D3D12Device.QueryInterface: Looking for ID3D12Device8, returning this" << std::endl;
		AddRef();
		*ppvObject = this;
		return S_OK;
	}
	else if (riid == __uuidof(ID3D12Device9))
	{
		...
		{
			Logger::LogInfo() << "D3D12Device.QueryInterface: m_device9 is not available, returning E_NOINTERFACE" << std::endl;
			return E_NOINTERFACE;
		}

		Logger::LogInfo() << "D3D12Device.QueryInterface: Looking for ID3D12Device9, returning this" << std::endl;
		AddRef();
		*ppvObject = this;
		return S_OK;
	}
	else if (riid == __uuidof(ID3D12Device10))
	{
		...
		{
			Logger::LogInfo() << "D3D12Device.QueryInterface: m_device10 is not available, returning E_NOINTERFACE" << std::endl;
			return E_NOINTERFACE;
		}

		Logger::LogInfo() << "D3D12Device.QueryInterface: Looking for ID3D12Device10, returning this" << std::endl;
		AddRef();
		*ppvObject = this;
		return S_OK;
	}
	...
	{
		Logger::LogInfo() << "D3D12Device.QueryInterface: Looking for ID3D12DeviceRemovedExtendedData" << std::endl;
		auto ret = m_device->QueryInterface(riid, ppvObject);
		Logger::LogInfo() << "D3D12Device.QueryInterface: Looking for ID3D12DeviceRemovedExtendedData result: " << std::endl;
		return ret;
	}
	...
	if (hr == S_OK && *ppvObject != nullptr)
	{
		...
		if (!wrapResult)
		{
			Logger::LogInfo() << "D3D12Device.QueryInterface: returning E_NOINTERFACE" << std::endl;
			return E_NOINTERFACE;
		}
	}

	Logger::LogInfo() << "D3D12Device.QueryInterface Unknown interface result: " << std::endl;
	return hr;
}
...
HRESULT __stdcall WrappedD3D12Device::CreateCommandQueue(const D3D12_COMMAND_QUEUE_DESC* pDesc, REFIID riid, void** ppCommandQueue)
{
	...
	HRESULT hr = m_device->CreateCommandQueue(pDesc, riid, (void**)&realQueue);
	if (FAILED(hr)) return hr;

	*ppCommandQueue = (ID3D12CommandQueue*)(new WrappedCommandQueue(realQueue));
	return S_OK;
}
...
HRESULT __stdcall WrappedD3D12Device::CreatePlacedResource2(ID3D12Heap* pHeap, UINT64 HeapOffset, const D3D12_RESOURCE_DESC1* pDesc, D3D12_BARRIER_LAYOUT InitialLayout, const D3D12_CLEAR_VALUE* pOptimizedClearValue, UINT32 NumCastableFormats, const DXGI_FORMAT* pCastableFormats, REFIID riid, void** ppvResource) 
{ 
